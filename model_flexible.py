...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.linear = nn.Linear(4*25088, num_classes)
    ...
    def forward(self, x, label=None, temperature=None, if_student = True):
        ...
        if label is not None:
            loss = torch.sum(nn.functional.softmax(label, -1)*nn.functional.log_softmax(out/temperature,-1), dim=1).mean()
            return -1.0*loss
        else:
    ...
    def predict(self, x, k, prob):
        ...
        for i,net in enumerate(self.weak_learners):
            if i = k:
                ...
                previous_prob += net.forward(x) * self.alpha[i]*self.gamma
                net.cpu()
