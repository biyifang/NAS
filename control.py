...
class StateSpace:
    ...
    def embedding_encode(self, id, value):
        ...
        one_hot[0, value_idx] = value_idx + 1 #changed from np.arange(1)
        print(state['name'], "one_hot:", one_hot)
        return one_hot
    ...
    def build_policy_network(self):
        with self.policy_session.as_default():
            ...
            with tf.compat.v1.name_scope('controller'):
                with tf.compat.v1.variable_scope('policy_network'):
                    ...
                    with tf.compat.v1.variable_scope('embeddings', reuse=tf.compat.v1.AUTO_REUSE):
                        ...
                        embeddings = tf.nn.embedding_lookup(params=embedding_weights[0], ids=state_input)
                    ...
                    for i in range(self.state_size * self.num_layers):
                        ...
                        with tf.compat.v1.name_scope('controller_output_%d' % i):
                            ...
                            cell_input = tf.argmax(input=preds, axis=-1)
                            ...
                            cell_input = tf.nn.embedding_lookup(params=embedding_weights[state_id], ids=cell_input,
            ...
            with tf.compat.v1.name_scope('losses'):
                ...
                tf.compat.v1.summary.scalar('discounted_reward', tf.reduce_sum(input_tensor=self.discounted_rewards))
                ...
                for i in range(self.state_size * self.num_layers):
                    ...
                    with tf.compat.v1.name_scope('state_%d' % (i + 1)):
                        ...
                        tf.compat.v1.summary.scalar('state_%d_ce_loss' % (i + 1), tf.reduce_mean(input_tensor=ce_loss))

                    cross_entropy_loss += ce_loss

                policy_gradient_loss = tf.reduce_mean(input_tensor=cross_entropy_loss)
                reg_loss = tf.reduce_sum(input_tensor=[tf.reduce_sum(input_tensor=tf.square(x)) for x in policy_net_variables])  # Regularization
